...
def process_csv_file(upload_id, file_path):
    ...
    Uses memory-efficient chunked processing to avoid memory issues
    """
    file_upload = None
    ...
    try:
        ...
        with open(full_file_path, 'r', encoding='utf-8') as csvfile:
            ...
            for row in reader:
                ...
                # Process chunk when it reaches chunk_size or at the end of file
                if len(chunk) >= chunk_size or processed_rows == total_rows:
                    # Process this chunk
                    with transaction.atomic():  # type: ignore
                        for chunk_row in chunk:
                            # Process each row in the chunk
                            sku = chunk_row.get('sku', '').upper()
                            name = chunk_row.get('name', '')
                            description = chunk_row.get('description', '')
                            price = chunk_row.get('price', 0)
